    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.callback != nil {
            self.callback?.dismiss(animated: true, completion: { () -> Void in
                if (self.callback?.responds(to: #selector(ViewController.call(_:))))! == true{
                    DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.background).async(execute: { () -> Void in
                        let view = self.callback as! ViewController
                        view.call(strTo)
                    })
                }
                
                if (self.callback?.responds(to: #selector(ViewController.connect(_:))))! == true{
                    DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.background).async(execute: { () -> Void in
                        let view = self.callback as! ViewController
                        view.connect(strTo)
                    })
